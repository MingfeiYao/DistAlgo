...
start(P_number, Num_p) ->
  receive
    {bind, ERB, Printer} -> 
      waitForTask(P_number, ERB, Num_p, Printer)
  end.
...
waitForTask(P_number, ERB, Num_p, Printer) ->
  receive
    {pl_deliver, Max_messages, Timeout} -> 
      ...
      ERB ! {broadcast, P_number},
      Initial_message = [ {P, 0} || P <- lists:seq(1, Num_p)],
      startTask(P_number, ERB, 1, Max_messages, maps:from_list(Initial_message), Printer)
  end.

startTask(P_number, ERB, Messages_sent, Max_messages, Messages, Printer) ->
  receive
    {message, Source_P_number} -> 
      case maps:is_key(Source_P_number, Messages) of
        true ->
          ...
          startTask(P_number, ERB, Messages_sent, Max_messages, New_Messages, Printer);
        false -> 
    ...
    after 0 ->
      if
        Messages_sent /= Max_messages ->
          ERB ! {broadcast, P_number},
          startTask(P_number, ERB, Messages_sent+1, Max_messages, Messages, Printer);
        true ->
          startTask(P_number, ERB, Messages_sent, Max_messages, Messages, Printer)
      end
  end.
